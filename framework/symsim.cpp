...
unsigned SymbolicExecutor::tracelen() const{
    return trace_.size();
}
...
void SymbolicExecutor::_check_only_invar(const smt::UnorderedTermMap & vdict) const {
    for (const auto & v : vdict) {
        if (invar_.find(v.first) == invar_.end())
            throw PonoException("Expecting " + v.first->to_string() + " to be input var");
    }
}
...
smt::UnorderedTermMap SymbolicExecutor::convert(const assignment_type & vdict) const{
...
void SymbolicExecutor::init(const smt::UnorderedTermMap & var_assignment /*={}*/){
    ...
    for (const auto & v : svar_){
        if (var_assignment_ref.find(v) == var_assignment_ref.end()){
            var_assignment_ref[v] = new_var(v->get_sort()->get_width(), v->to_string(), false);
        }
    }
...
void SymbolicExecutor::set_input(const smt::UnorderedTermMap & invar_assign, const smt::TermVec & pre_assumptions) {
    ...
    history_choice_.push_back(ChoiceItem(pre_assumptions, invar_assign)); // construct by r-value reference
    auto & c = history_choice_.back();

    for(const auto & v : invar_){
         if (prev_sv.find(v) != prev_sv.end()){
            cout << "WARNING: shadowing input assignment as assigned by prev-state " << v->to_string() << endl;
         }
         if (c.var_assign_.find(v) == c.var_assign_.end()){
            c.var_assign_[v] = new_var(v->get_sort()->get_width(), v->to_string(), true);
         }
    }
...
void SymbolicExecutor::undo_set_input(){
    ...
    history_choice_.pop_back(); // you can only pop in the end, o.w. reference will fail
}


/// similar to cur(), but will check no reference to the input variables
smt::Term SymbolicExecutor::interpret_state_expr_on_curr_frame(const smt::Term & expr) const {
    if(!_expr_only_sv(expr))
...
/// similar to cur(), but will check no reference to the input variables
smt::TermVec SymbolicExecutor::interpret_state_expr_on_curr_frame(const smt::TermVec & expr_list) const {
    smt::TermVec ret;
...
void SymbolicExecutor::sim_one_step() {
    ...
    trace_.push_back(std::move(svmap)); // svmap will not be used afterwards, avoid copy
...
smt::Term SymbolicExecutor::new_var(int bitwdth, const std::string & vname /*"=var"*/, bool x /*=true*/) {
