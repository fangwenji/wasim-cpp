...
cdef extern from "framework/ts.h" namespace "wasim":
    cdef cppclass StateAsmpt:
        StateAsmpt(const c_UnorderedTermMap & sv, const c_TermVec & asmpt, const vector[string] & assumption_interp)  except +
        string print() except +
        string print_assumptions() except +
        bint syntactically_contains_x(const c_UnorderedTermSet & set_of_Xvar) except +
        bint is_reachable(const c_SmtSolver & s) except +
        const c_UnorderedTermMap & get_sv() except +
        const c_TermVec & get_assumptions() except +
        const vector[string] & get_assumption_interpretations() except +
...
cdef extern from "framework/symtraverse.h" namespace "wasim":
    cdef cppclass TraverseBranchingNode:
        TraverseBranchingNode();
