...
int main(){
    ...
    auto asmpt_prop = solver->make_term(smt::Implies, tag3, \
                        solver->make_term(smt::Equal, reg_v_comp, \
                            solver->make_term(smt::BVAdd, solver->make_term(1, reg_v->get_sort()), \
                                solver->make_term(smt::BVMul, reg_v, \
                                    solver->make_term(2, reg_v->get_sort())))));
    auto asmpt = solver->make_term(smt::And, {asmpt_rst1, asmpt_rst2, asmpt_tag, asmpt_prop});
    ...
    for(const auto& prop:inv_l4_prop){
        inv_prop1.push_back(solver->make_term(smt::Implies, tag3, \
                                solver->make_term(smt::Equal, reg_v, prop)));
    }
    ...
    for(int i=0; i<inv_vec4.size(); i++){
        inv_prop2.push_back(solver->make_term(smt::And, \
    ...
    smt::Term inv = {}; 
    inv = solver->make_term(smt::And, smt::TermVec{inv_tag0, inv_tag1, inv_tag2_p1, inv_tag3_p2, inv_prop_expr});
    // define inv end
    ...
    auto init = solver->make_term(smt::And, smt::TermVec{init_sts, \
                    ...
                    solver->make_term(smt::Equal, solver->substitute(rst, sts.state_updates()), solver->make_term(0))});
