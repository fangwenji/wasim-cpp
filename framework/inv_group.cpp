...
void InvGroup::branch2state(){
    ...
    for (const auto& asmpt_expr:asmpt_group_){
        if(std::find(asmpt_dedup_.begin(), asmpt_dedup_.end(), asmpt_expr) != asmpt_dedup_.end()){
            // found state_expr in state_dedup
            continue;
        }
        else{
...
smt::TermVec InvGroup::asmpt_group(){
    return asmpt_group_;
}
...
std::vector<int> InvGroup::check_unsat_expr(std::vector<int> truelist_num, smt::Term cex_expr){
    ...
    asmpt_group_.clear();
    asmpt_group_.insert(asmpt_group_.end(), asmpt_group_temp.begin(), asmpt_group_temp.end());

    /// check truelist
    std::vector<int> truelist_num_ret = {};
    for(const auto& num:truelist_num){
        auto asmpt_sat_check = solver_->make_term(smt::And, asmpt_group_.at(num), cex_expr);
...
void InvGroup::add_unsat_asmpt(smt::Term unsat_expr, int idx){
    ...
    state_group_.clear();
    state_group_.insert(state_group_.end(), state_group_temp.begin(), state_group_temp.end());
...
check_res init_check(smt::Term init, smt::Term inv, smt::Term asmpt/*={}*/, smt::SmtSolver& solver){
...
check_res inv_check(smt::Term inv, smt::Term trans_update, smt::Term asmpt/*={}*/, TransitionSystem sts, smt::SmtSolver& solver){
    auto LHS = solver->make_term(smt::And, smt::TermVec{inv, asmpt, trans_update});
...
check_res prop_check(smt::Term prop, smt::Term inv, smt::Term asmpt, smt::SmtSolver& solver){
...
smt::TermVec deduplicate(smt::TermVec vec){
    ...
    for (const auto& expr:vec){
        if(std::find(dedup_vec.begin(), dedup_vec.end(), expr) != dedup_vec.end()){
            // found state_expr in state_dedup
            continue;
        }
        else{
...
cex_res cex_parser(smt::UnorderedTermMap cex, std::unordered_set<std::string> tag_set, std::unordered_set<std::string> ctrl_set, smt::SmtSolver& solver){
    //1. get current tag
    smt::Term tag_ret = {};
...
std::vector<int> test_cex(smt::TermVec formula_vec, smt::UnorderedTermMap cex, smt::SmtSolver& solver){
